...
from cybulldetection.utils.utils import SpellCorrectionModel
...
class DatasetCleaner(ABC):
    ...
    def clean_text(self, text: str) -> str:
        """
        Cleans the given string of text
        """
        pass
    ...
    def clean_words(self, words: list[str]) -> list[str]:
        """
        Cleans each word in the list of words
        """
        pass
...
class PunctuationDatasetCleaner(DatasetCleaner):
    def __init__(self, punctuation: str = string.punctuation) -> None:  # default punc from string module
        super().__init__()
    ...
    def clean_words(self, words: list[str]) -> list[str]:
        # this will remove the punctuations and replace with no-space character
...
class ReTweetDatasetCleaner(DatasetCleaner):
    def clean_text(self, text: str) -> str:
        return re.sub(r"\bRT\b", "", text, flags=re.IGNORECASE)  # replacing words which starts with RT
...
class SpellCorrectionDatasetCleaner(DatasetCleaner):
    ...
    def clean_text(self, text: str) -> str:
        return self.spell_correction_model(text)  # will call the __call__ method()
...
class DatasetCleanerManager:
    ...
    def __call__(self, text: str | list[str]) -> str | list[str]:
        # calls all the dataset cleaner / preprocessing method classes
        for dataset_cleaner in self.dataset_cleaners.values():
            text = dataset_cleaner(
                text
            )  # which invokes base class __call__() method to call either clean_text() or clean_words()
        return text
