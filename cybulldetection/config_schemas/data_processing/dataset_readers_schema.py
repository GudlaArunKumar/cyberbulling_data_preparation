...
class GHCDatasetReaderConfig(DatasetReaderConfig):
    _target_: str = "cybulldetection.data_processing.dataset_readers.GHCDatasetReader"
    val_split_ratio: float = MISSING

    # other two parameters dataset_dir and dataset_name are initiated from above base class.


@dataclass
class JigsawToxicCommentsDatasetReaderConfig(DatasetReaderConfig):
    _target_: str = "cybulldetection.data_processing.dataset_readers.JigsawToxicCommentsDatasetReader"
    val_split_ratio: float = MISSING


@dataclass
class TwitterDatasetReaderConfig(DatasetReaderConfig):
    _target_: str = "cybulldetection.data_processing.dataset_readers.TwitterDatasetReader"
    val_split_ratio: float = MISSING
