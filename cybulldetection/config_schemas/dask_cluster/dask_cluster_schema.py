...
class LocalDaskClusterConfig(DaskClusterConfig):
    _target_: str= "dask.distributed.LocalCluster"  # paramters takemn from dask LocalCluster docs 
    memory_limit: int = 0  # "auto" is causing memory limit error so changed it to 0
    processes: bool = True 
...
class GCPDaskClusterConfig(DaskClusterConfig):
    _target_: str = "dask_cloudprovider.gcp.GCPCluster"   # from the documentation
    ...
    extra_bootstrap: Optional[list[str]] = field(
        default_factory=lambda: ["gcloud auth configure-docker --quiet europe-west4-docker.pkg.dev"]  # to authenticate the artifact registry
    )
