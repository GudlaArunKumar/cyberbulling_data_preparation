...
def get_nrof_partitions(
    ...
    if available_memory is None:
        ...
        available_memory = available_memory * no_of_workers

    if df_memory_usage <= min_partition_size:
        return 1

    if df_memory_usage / no_of_workers <= min_partition_size:
    ...
    nrof_partitions = nrof_partitions_per_worker * no_of_workers

    while (df_memory_usage / (nrof_partitions + 1)) > min_partition_size and (
        nrof_partitions // no_of_workers
...
def repartition_dataframe(
    ...
    nrof_partitions = get_nrof_partitions(
        df_memory_usage, no_of_workers, available_memory, min_partition_size, aimed_nrof_partitions_per_worker
    )
    partitioned_df: dd.core.DataFrame = df.repartition(npartitions=1).repartition(npartitions=nrof_partitions)  # type: ignore
    return partitioned_df


def get_repo_address_with_access_token(gcp_project_id: str, gcp_secret_id: str, repo_address: str, user_name: str):
...
def filter_based_on_minimum_number_of_words(df: pd.DataFrame, min_no_of_words: int) -> pd.DataFrame:
    return df[df["cleaned_text"].str.split().apply(len) >= min_no_of_words]